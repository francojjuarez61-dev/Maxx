...
function isPointInsideCircleClick(e, el){
  // Si viene de teclado/assistive (sin coords), lo dejamos pasar
...
ringBtn.addEventListener('click', (e) => {
  if (!isPointInsideCircleClick(e, ringBtn)) return;
...
// =========================
// PATCH 2026-01-20: Apariencia + fondos + hit-test del botón
// =========================
const APPEAR_KEY = 'barber_appearance_v2';
const APPEAR_KEY_OLD = 'barber_appearance_v1';
...
function loadAppearance(){
  try{
    ...
    // Migración suave desde v1 (si existe)
    const old = localStorage.getItem(APPEAR_KEY_OLD);
    if (old){
      const o = JSON.parse(old) || {};
      const migrated = {
        theme: o.theme || 'dark',
        btnBg: o.btnBg || null,
        btnBgScale: (o.btnBgScale ?? 1),
        // v1 guardaba px; lo normalizamos a 0 (centrado) para evitar desalineaciones
        btnBgXn: 0,
        btnBgYn: 0
      };
      try{ localStorage.setItem(APPEAR_KEY, JSON.stringify(migrated)); }catch(_){}
      return migrated;
    }
    return {};
  }catch(_){ return {}; }
...
function saveAppearance(obj){
  try{ localStorage.setItem(APPEAR_KEY, JSON.stringify(obj)); }catch(_){}
}

let appearance = loadAppearance();

function cssUrlOrNone(dataUrl){
...
function applyAppearance(){
  ...
  // button bg (normalizado para que no se desacomode entre pantallas)
  document.documentElement.style.setProperty('--btn-bg-url', cssUrlOrNone(appearance.btnBg));
  document.documentElement.style.setProperty('--btn-bg-scale', String(appearance.btnBgScale ?? 1));

  const rect = ringBtn?.getBoundingClientRect?.() || { width: 320, height: 320 };
  // ring-bg usa inset ~10% => área útil ~80%
  const areaW = Math.min(rect.width, rect.height) * 0.80;

  const xn = appearance.btnBgXn ?? 0;
  const yn = appearance.btnBgYn ?? 0;

  const pxX = xn * areaW;
  const pxY = yn * areaW;

  document.documentElement.style.setProperty('--btn-bg-x', pxX.toFixed(2) + 'px');
  document.documentElement.style.setProperty('--btn-bg-y', pxY.toFixed(2) + 'px');
}
...
function updateAppearanceThumbs(){
  ...
  if (darkBtn && lightBtn){
    const t = appearance.theme || 'dark';
    darkBtn.classList.toggle('is-active', t === 'dark');
    lightBtn.classList.toggle('is-active', t === 'light');
  }
}

// ===== Editor modal =====
const bgEditorModal = document.getElementById('bgEditorModal');
const bgEditorTitle = document.getElementById('bgEditorTitle');
const bgEditorSub = document.getElementById('bgEditorSub');
...
const bgCenterBtn = document.getElementById('bgCenterBtn');
...
let bgTarget = null; // 'btn'
let draft = { dataUrl:null, x:0, y:0, scale:1 };
let dragging = false;
let dragStart = { x:0, y:0, ox:0, oy:0 };

function openBgEditor(target){
  bgTarget = target;
  if (!bgEditorModal) return;

  const isRingBtn = true;

  if (bgEditorTitle) bgEditorTitle.textContent = 'Fondo del botón central';
  if (bgEditorSub) bgEditorSub.textContent = 'Ajustá cómo se ve dentro del círculo';

  bgPreview?.classList.add('is-circle');

  // load current (normalized)
  draft.dataUrl = appearance.btnBg || null;
  draft.scale = appearance.btnBgScale ?? 1;

  // para no desacomodar: si venís de v1 o no hay valores, queda centrado
  const xn = appearance.btnBgXn ?? 0;
  const yn = appearance.btnBgYn ?? 0;

  if (bgScale) bgScale.value = String(draft.scale);
  if (bgPreviewImg){
    bgPreviewImg.src = draft.dataUrl || '';
    bgPreviewImg.style.display = draft.dataUrl ? 'block' : 'none';
  }

  openAnyModal(bgEditorModal);

  // una vez visible, calculamos px para la previsualización
  requestAnimationFrame(()=>{
    const w = bgPreview?.clientWidth || 320;
    const area = w; // preview cuadrado
    draft.x = xn * area;
    draft.y = yn * area;
    renderDraft();
  });
...
function closeBgEditor(){
  if (!bgEditorModal) return;
  closeAnyModal(bgEditorModal);
  bgTarget = null;
  draft = { dataUrl:null, x:0, y:0, scale:1 };
  dragging = false;
...
function renderDraft(){
  ...
  const t = `translate3d(-50%,-50%,0) translate3d(${draft.x}px, ${draft.y}px, 0) scale(${draft.scale})`;
  bgPreviewImg.style.transform = t;
}
...
function setDraftImage(dataUrl){
  draft.dataUrl = dataUrl;
  draft.x = 0; draft.y = 0; draft.scale = 1;
...
function computeFitScale(){
  if (!bgPreview || !bgPreviewImg || !draft.dataUrl) return 1;
  const box = bgPreview.getBoundingClientRect();
  // Natural size might not be loaded yet; fallback
  const iw = bgPreviewImg.naturalWidth || box.width;
  const ih = bgPreviewImg.naturalHeight || box.height;
...
bgScale?.addEventListener('input', ()=>{
  draft.scale = parseFloat(bgScale.value) || 1;
  renderDraft();
});

bgCenterBtn?.addEventListener('click', ()=>{
  draft.x = 0; draft.y = 0;
  renderDraft();
});
...
bgFitBtn?.addEventListener('click', ()=>{
  ...
  draft.scale = Math.min(3, Math.max(0.5, s));
  if (bgScale) bgScale.value = String(draft.scale);
  draft.x = 0; draft.y = 0;
  renderDraft();
});

bgCancelBtn?.addEventListener('click', closeBgEditor);
bgEditorCloseBtn?.addEventListener('click', closeBgEditor);

bgSaveBtn?.addEventListener('click', ()=>{
  if (bgTarget !== 'btn') return;

  appearance.btnBg = draft.dataUrl;
  appearance.btnBgScale = draft.scale;

  // Normalizar offsets para evitar que se rompa el layout al cambiar de tamaño
  const w = bgPreview?.clientWidth || 320;
  const h = bgPreview?.clientHeight || 320;
  const area = Math.min(w, h) || 320;

  const xn = (draft.x || 0) / area;
  const yn = (draft.y || 0) / area;

  // clamp razonable
  appearance.btnBgXn = Math.max(-0.5, Math.min(0.5, xn));
  appearance.btnBgYn = Math.max(-0.5, Math.min(0.5, yn));
...
// Dragging
bgPreview?.addEventListener('pointerdown', (e)=>{
  if (!draft.dataUrl) return;
  dragging = true;
  bgPreview.setPointerCapture(e.pointerId);
  dragStart.x = e.clientX;
  dragStart.y = e.clientY;
  dragStart.ox = draft.x;
  dragStart.oy = draft.y;
});

bgPreview?.addEventListener('pointermove', (e)=>{
  if (!dragging) return;
  const dx = e.clientX - dragStart.x;
  const dy = e.clientY - dragStart.y;
  draft.x = dragStart.ox + dx;
  draft.y = dragStart.oy + dy;
  renderDraft();
});

bgPreview?.addEventListener('pointerup', ()=>{
  dragging = false;
});
bgPreview?.addEventListener('pointercancel', ()=>{
  dragging = false;
});

// ===== Apariencia controls inside settings =====
...
clearBtnBgBtn?.addEventListener('click', ()=>{
  ...
  appearance.btnBgScale = 1;
  appearance.btnBgXn = 0;
  appearance.btnBgYn = 0;
  saveAppearance(appearance);
  applyAppearance();
  updateAppearanceThumbs();
});
...
// Ensure ring click only comes from the ring itself (extra safety)
ringBtn?.addEventListener('pointerdown', (e)=> e.stopPropagation(), { passive: true });
ringBtn?.addEventListener('touchstart', (e)=> e.stopPropagation(), { passive: true });